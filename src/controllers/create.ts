...
import { BadRequestError, IMessageDocument, uploads } from '@eoladapo/jobman-shared';
...
const message = async (req: Request, res: Response): Promise<void> => {
  ...
  if (file) {
    ...
    if (!result?.public_id) {
      throw new BadRequestError('File upload error. Try again.', 'Message create() method error');
    }
